...
from src.models import HotelsOrm, ImagesOrm
...
def process_image(_self: Task, hotel_id: int, original_filename: str, temp_file_path: str, db_name: str) -> dict:
    ...
    try:
        ...
        return {"status": "error", "message": "Ошибка при обработке изображения"}


def _create_image_in_db_sync(
    hotel_id: int, original_filename: str, width: int, height: int, db_name: str
) -> int | None:
    ...
    try:
        ...
        image = ImagesOrm(filename=original_filename, original_filename=original_filename, width=width, height=height)
        session.add(image)
