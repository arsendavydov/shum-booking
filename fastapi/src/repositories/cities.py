...
class CitiesRepository(BaseRepository[CitiesOrm]):
    ...
    async def get_paginated(
        self, page: int, per_page: int, name: str | None = None, country_id: int | None = None
    ) -> list[SchemaCity]:
        """
        ...
        query = select(self.model).options(selectinload(self.model.country))
    ...
    async def get_by_id(self, id: int) -> SchemaCity | None:
        """
        ...
        query = select(self.model).options(selectinload(self.model.country)).where(self.model.id == id)
        result = await self.session.execute(query)
    ...
    async def get_by_id_orm(self, id: int) -> CitiesOrm | None:
        """
    ...
    async def get_by_name_and_country_id(self, name: str, country_id: int) -> CitiesOrm | None:
        """
        ...
        query = (
            select(self.model)
            .options(selectinload(self.model.country))
            .where(self.model.name == name, self.model.country_id == country_id)
        )
        result = await self.session.execute(query)
        return result.scalar_one_or_none()

    async def get_by_name_case_insensitive(self, name: str) -> CitiesOrm | None:
        """
    ...
    async def create_city_with_validation(self, name: str, country_id: int) -> SchemaCity:
        """
    ...
    async def update_city_with_validation(self, city_id: int, name: str, country_id: int) -> SchemaCity:
        """
        ...
        # Обновляем город
        updated_city = await self.edit(id=city_id, name=name, country_id=country_id)
    ...
    async def partial_update_city_with_validation(
        ...
        query = select(self.model).options(selectinload(self.model.country)).where(self.model.id == city_id)
        result = await self.session.execute(query)
