...
async def get_facilities(
    ...
    facilities = await repo.get_paginated(page=pagination.page, per_page=pagination.per_page, title=title)
    return facilities
...
async def get_facility_by_id(
    facility_id: int = Path(..., description="ID удобства"), db: DBDep = DBDep
) -> SchemaFacility:
    """
...
async def create_facility(
    ...
    facility: Facility = Body(
        ..., openapi_examples={"1": {"summary": "Создать удобство", "value": {"title": "Wi-Fi"}}}
    ),
) -> MessageResponse:
    """
