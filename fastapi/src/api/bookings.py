...
async def get_bookings(pagination: PaginationDep, db: DBDep) -> list[SchemaBooking]:
    """
    ...
    bookings = await repo.get_paginated(page=pagination.page, per_page=pagination.per_page)
    return bookings
...
async def get_my_bookings(pagination: PaginationDep, db: DBDep, current_user: CurrentUserDep) -> list[SchemaBooking]:
    """
    ...
    """
    repo = DBManager.get_bookings_repository(db)
    bookings = await repo.get_paginated(page=pagination.page, per_page=pagination.per_page, user_id=current_user.id)
    return bookings
...
async def create_booking(db: DBDep, current_user: CurrentUserDep, booking: Booking = Body(...)) -> MessageResponse:
    """
    ...
    async with DBManager.transaction(db):
        ...
        try:
            await bookings_repo.create_booking_with_validation(
                room_id=booking.room_id, user_id=current_user.id, date_from=booking.date_from, date_to=booking.date_to
            )
...
async def delete_booking(booking_id: int, db: DBDep, current_user: CurrentUserDep) -> MessageResponse:
    """
